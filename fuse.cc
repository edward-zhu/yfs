                   ...
                   int to_set, struct fuse_file_info *fi)
{
  ...
  if (FUSE_SET_ATTR_SIZE & to_set) {
    ...
    int ret;
    if ((ret = yfs->resize(ino, attr->st_size)) != yfs_client::OK) {
                ...
                off_t off, struct fuse_file_info *fi)
{
  ...
  if ((ret = yfs->read(ino, size, off, buf)) != yfs_client::OK) {
    if (ret == yfs_client::NOENT) fuse_reply_err(req, ENOENT);
    fuse_reply_err(req, EIO);
  }
                 ...
                 struct fuse_file_info *fi)
{
  ...
  yfs_client::status ret;
  size_t nsize;
  if ((ret = yfs->write(ino, buf, size, off, &nsize)) != yfs_client::OK) {
    if (ret == yfs_client::NOENT) fuse_reply_err(req, ENOENT);
    fuse_reply_err(req, EIO);
    return;
  }
  printf("[FUSE] write file succeed with size %lu.\n", nsize);
  fuse_reply_write(req, nsize);
#else
  fuse_reply_err(req, ENOSYS);
#endif
}
                        ...
                        mode_t mode, struct fuse_entry_param *e)
{
  ...
  printf("[FUSE] create file in dir %016llx\n", parent);

  // pick a inum
  yfs_client::inum inum = pick_new_file_inum();

  printf("[FUSE] file inum %016llx\n", inum);

  // insert into parent dir
  yfs_client::status ret = yfs->create(inum, name, parent);
  if (ret != yfs_client::OK) {
    return ret;
  }
...
fuseserver_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  if ((ret = yfs->lookup(parent, name, &inum)) != yfs_client::OK) {
      if (ret == yfs_client::NOENT) {
        printf("lookup undefined error : %d\n", ret);
      }
      found = false;
  }
  ...
  if (found) {
    if ((ret = getattr(inum, e.attr)) != yfs_client::OK) {
      printf("[FUSE] lookup failed when getattr: %lu\n", ret);
      fuse_reply_err(req, ENOENT);
    }
    e.ino = inum;
    fuse_reply_entry(req, &e);
    ...
    printf("[FUSE] lookup return not found.\n");
    fuse_reply_err(req, ENOENT);
  }
}
                   ...
                   off_t off, struct fuse_file_info *fi)
{
  ...
  if ((ret = yfs->readdir(ino, ents)) != yfs_client::OK) {
    fuse_reply_err(req, ENOENT);
  }

  for (auto && ent : ents) {
    dirbuf_add(&b, ent.name.c_str(), ent.inum);
  }
